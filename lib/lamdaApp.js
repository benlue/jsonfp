...
function exeProg(ctx, p)  {
    ...
    var  f = knownMethods[opName],
         rtn = null;
    if (f)  {
        var  option = evalOption(ctx, p[opName]);
        rtn = f.call(ctx, option);
    }
    else
 ...
 */
function doEval(param)  {
    var  obj = {},
         ctx = this;

    _.keys(param).
    map(function(k) {
        obj[k] = exeProg( ctx, param[k] );
    })
...
function doIntersection(another)  {
    another = evalObj(this, another);
    return _.intersection(this.list, another);
};
...
function  doPick(selectors)  {
    selectors = evalObj(this, selectors);
...
/**
 * If the option is not really a plain object but a 'eval' JSON-fp program,
 * we'll evaluate it before feeding the option to its operator.
 */
function evalOption(ctx, option)  {
    if (_.isPlainObject(option) && option.eval && _.size(option) === 1)
        option = knownMethods['eval'].call(ctx, option.eval);
    return  option;
}
